...
function tooltips.drawBtn(gtaTravel, label, key)
    ...
    if active then
        ...
        elseif key == "resetStuff" then
            gtaTravel.CPS.CPToolTip1Begin(260, 143)
            ImGui.TextWrapped("Reset Cam:\nThis resets your cam to its deault position and also removes any movement locks")
            ImGui.Separator()
            ImGui.TextWrapped("Remove Restrictions:\nUse this to remove any gameplay restrictions such as no weapons and no zoom, that get applied during the transition")
            gtaTravel.CPS.CPToolTip1End()
